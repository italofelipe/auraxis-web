...
import TesouroDiretoPage from "./tesouro-direto.vue";
import type { TesouroDiretoResult } from "~/features/tools/model/tesouro-direto";
...
vi.mock("~/features/tools/model/tesouro-direto", () => ({
  TESOURO_TABLE_YEAR: 2025,
  TESOURO_TYPES: ["selic", "ipca_plus", "prefixado"],
  createDefaultTesouroDiretoFormState: (): object => ({
    type: "selic",
    amount: null,
    termDays: 365,
  ...
  validateTesouroDiretoForm: mockValidate,
  calculateTesouroDireto: mockCalculate,
}));
...
function nuxtContextPlugin(app: App): void {
  const fakeNuxtApp = {
    _route: { path: "/tools/tesouro-direto", meta: {}, params: {}, query: {} },
...
describe("TesouroDiretoPage — guest layout", () => {
...
describe("TesouroDiretoPage — authenticated layout", () => {
  beforeEach(() => { resetPageState(); });
...
describe("TesouroDiretoPage — form validation", () => {
  beforeEach(() => { resetPageState(); });
  ...
  it("shows validation error when amount is missing", async () => {
    ...
    expect(wrapper.text()).toContain("tesouroDireto.errors.amountRequired");
    expect(mockCalculate).not.toHaveBeenCalled();
  });

  it("shows validation error when taxaIndicativa is missing", async () => {
    mockValidate.mockReturnValue([{ field: "taxaIndicativaPct", messageKey: "errors.taxaIndicativaRequired" }]);
    const wrapper = mountPage();
    await wrapper.find("form").trigger("submit");
    await flushPromises();
    expect(wrapper.text()).toContain("tesouroDireto.errors.taxaIndicativaRequired");
  });
  ...
  it("clears validation error on a valid submit", async () => {
    ...
    await wrapper.find("form").trigger("submit");
    await flushPromises();
    expect(wrapper.text()).not.toContain("tesouroDireto.errors.amountRequired");
  });
});

describe("TesouroDiretoPage — calculation results", () => {
  beforeEach(() => { resetPageState(); });
  ...
  it("shows CalculatorResultSummary after valid calculation", async () => {
    ...
    expect(wrapper.text()).toContain("tesouroDireto.results.netAmount");
  });

  it("shows breakdown section after calculation", async () => {
    mockCalculate.mockReturnValue(mockResult);
    const wrapper = mountPage();
    await wrapper.find("form").trigger("submit");
    await flushPromises();
    expect(wrapper.find(".tesouro-page__breakdown").exists()).toBe(true);
  });
  ...
  it("shows disclaimer warning after calculation", async () => {
    ...
    await wrapper.find("form").trigger("submit");
    await flushPromises();
    expect(wrapper.text()).toContain("tesouroDireto.disclaimer.note");
  });

  it("does not show results before calculation", () => {
    const wrapper = mountPage();
    expect(wrapper.find(".calculator-result-summary").exists()).toBe(false);
    expect(wrapper.find(".tesouro-page__breakdown").exists()).toBe(false);
  });
});

describe("TesouroDiretoPage — save simulation", () => {
  beforeEach(() => { resetPageState(); });

  it("calls saveSimulationMutation with tesouro_direto slug", async () => {
    mockIsAuthenticated.value = true;
    mockCalculate.mockReturnValue(mockResult);
    mockSaveMutateAsync.mockResolvedValue({ id: "sim-t-001" });
    const wrapper = mountPage();
    await wrapper.find("form").trigger("submit");
    await flushPromises();
    const saveButton = wrapper.findAll(".n-button").find((b) => b.text().includes("tesouroDireto.actions.save"));
...
describe("TesouroDiretoPage — premium add as goal", () => {
  beforeEach(() => { resetPageState(); });
  ...
  it("shows add as goal button for premium users", async () => {
    ...
    await wrapper.find("form").trigger("submit");
    await flushPromises();
    const goalButton = wrapper.findAll(".n-button").find((b) => b.text().includes("tesouroDireto.actions.addAsGoal"));
  ...
  it("calls createGoalMutation when premium user adds goal", async () => {
    ...
    mockHasPremiumAccess.value = true;
    mockCalculate.mockReturnValue(mockResult);
    mockSaveMutateAsync.mockResolvedValue({ id: "sim-t-002" });
    mockCreateGoalMutateAsync.mockResolvedValue({ id: "goal-t-1" });
    const wrapper = mountPage();
    await wrapper.find("form").trigger("submit");
    await flushPromises();
    const goalButton = wrapper.findAll(".n-button").find((b) => b.text().includes("tesouroDireto.actions.addAsGoal"));
