...
export default async (): Promise<UserConfig> => {
  ...
  return {
    ...
    // — worker fork morto por pressão de memória no runner (OOM do container,
    // sem "JS heap out of memory"). Era flaky no job SonarCloud, que regenera o
    // coverage com `pnpm test:coverage`. Local mantém paralelismo total.
    // No Vitest 4 `pool`/`poolOptions` são TOP-LEVEL (não `test.poolOptions`).
    pool: "forks",
    poolOptions: {
